...
public class UserDao {
	...
	public static boolean update(User users) throws SQLException,NamingException {
		...
		try {
			...
			stmt = conn.prepareStatement("UPDATE users"
					+ "SET id=?, pwd=?, address=?, phone=?" + "WHERE id=?");
	...
	public static boolean create(User users) throws SQLException,NamingException {
		...
		try {
			conn = ds.getConnection();
			stmt = conn	.prepareStatement("INSERT INTO users(id, pwd, name, address, phone)"
							+"VALUES(?,?,?,?,?)");
			stmt.setString(1, users.id);
			stmt.setString(2, users.pwd);
			stmt.setString(3, users.name);
			stmt.setString(4, users.address);
			stmt.setInt(5, users.phone);
	...
	public static User findById(int id) throws NamingException, SQLException {
		...
		try {
			...
			if (rs.next()) {
				users = new User(rs.getString("id"), rs.getString("pwd"), rs.getString("name"),
	...
	public static PageResult<User> getPage(int page, int numIntemsInpage)
			throws SQLException, NamingExceptino {
		Connection conn = null;
		Statement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			stmt = conn.createStatement();
			rs = stmt.executeQuery("SELECT * FROM users ORDER BY name LIMIT "
					+ startPos + ", " + numItemsInpage);

			while (rs.next()) {
				result.getList().add(new User(rs.getString("id")),
						rs.getString("pwd"), rs.getString("address"),
						rs.getInt("phone"));
			}
		} finally {
			if (rs != null)
